...
def massage2canonical_stage1(expr, cases, levels):
    ...
    if len(matches) > 0:
        ...
        expr.children = [massage2canonical_stage1(
            child, cases, levels+[1]) for child_index, child in enumerate(expr.children)]
        levels[-1] += 1
        ...
        # print_structure(prev_expr)
        expr.children = [massage2canonical_stage1(
            child, cases, levels+[child_index+1]) for child_index, child in enumerate(expr.children)]
        pass

    if False:
        if isinstance(expr, AddExpr):
            left, right = expr.children[0], expr.children[1]
            expr = massage2canonical_stage1(
                left) + massage2canonical_stage1(right)
            ...
            if left.contains(DifferentialExpr):
                if isinstance(left, AddExpr):
                    ...
                    expr = massage2canonical_stage1(
                        A*C) + massage2canonical_stage1(B*C)
            ...
            if right.contains(DifferentialExpr):
                if isinstance(right, AddExpr):
                    ...
                    expr = massage2canonical_stage1(
                        A*B) + massage2canonical_stage1(A*C)
            ...
            if isinstance(child, MatMulExpr):
                ...
                if A.contains(DifferentialExpr):  # Move to right side
                    expr = massage2canonical_stage1(
                        Tr(massage2canonical_stage1(B*massage2canonical_stage1(A))))
                # Change associativity so that dX becomes on the right-most side of left
                elif B.contains(DifferentialExpr) and not isinstance(B, DifferentialExpr):
                    if isinstance(B, MatMulExpr):
                        ...
                        expr = massage2canonical_stage1(Tr(massage2canonical_stage1(
                            A*massage2canonical_stage1(B))*massage2canonical_stage1(C)))
                    elif isinstance(B, TransposeExpr):
                        # Tr(AB') = Tr(A'B)
                        expr = massage2canonical_stage1(
                            Tr(massage2canonical_stage1((A.T)*massage2canonical_stage1(B.T))))
            elif isinstance(child, AddExpr):
                # Tr(A+B) = Tr(A) + Tr(B)
                A, B = child.children[0], child.children[1]
                expr = massage2canonical_stage1(massage2canonical_stage1(
                    Tr(A)) + massage2canonical_stage1(Tr(B)))
